...
import { createSeller } from "@/app/(protected)/onboarding/actions";
...
export async function createStripeAccountLink() {
  ...
  if (stripeAccount) {
    const accountLink = await stripe.accountLinks.create({
      ...
      refresh_url: `${process.env.NEXT_PUBLIC_APP_URL}/config/billing`,
      return_url: `${process.env.NEXT_PUBLIC_APP_URL}/config/billing`,
...
export async function onboardingCreateStripeAccountLink() {
  ...
  if (stripeAccount) {
    const accountLink = await stripe.accountLinks.create({
      account: stripeAccount.stripe_account_id as string,
      refresh_url: `${process.env.NEXT_PUBLIC_APP_URL}/onboarding?step=3`,
      return_url: `${process.env.NEXT_PUBLIC_APP_URL}/onboarding?step=3`,
      type: "account_onboarding",
    });
