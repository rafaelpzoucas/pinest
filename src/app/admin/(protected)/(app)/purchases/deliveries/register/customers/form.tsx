...
type CustomersFormProps = {
  ...
  closeSheet: () => void
  purchaseForm: UseFormReturn<z.infer<typeof createPurchaseFormSchema>>
...
export function CustomersForm({
  ...
  const { mutate: createCustomer, isPending: isCreating } = useMutation({
    ...
    onSuccess: ({ createdStoreCustomer }) => {
      ...
      purchaseForm.setValue('customer_id', newCustomer.id)
      closeSheet()
    },
    onError: ({ message }) => {
  ...
  useEffect(() => {
    if (isUpdate) {
      purchaseForm.setValue('customer_id', selectedCustomer.id)
